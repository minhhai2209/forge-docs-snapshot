...
Even if you have upgraded your Forge app to the latest version of UI Kit, please be aware that sites using your Forge app might not be using this version. This could be the case if you released it as a major version change. For apps not using [Rolling releases](/platform/forge/rolling-releases/), site admins must manually update their site to use the new major version.

Apps using Rolling releases can roll out app code to installations before admins approve new permissions. Permission changes still require admin approval, and you can monitor code rollouts in [View app rollouts](/platform/forge/view-app-rollouts/).
