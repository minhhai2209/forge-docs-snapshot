...
The Custom Entity Store lets you store data in
[custom entities](/platform/forge/storage-reference/entities-manifest/),
which are data structures you can define according to your app's needs. Custom entities let you assign multiple values (or
"attributes") to a single key (or "entity") and define indexes to optimize queries against these values.
...
In addition, the Atlassian Cloud backs up all [persistent](/platform/forge/storage-reference/#persistent) storage for disaster recovery.
...
Keys are lexicographically ordered; this means, for example, that the [Query builder](/platform/forge/storage-reference/kvs-api-query/) and [Complex Query builder](/platform/forge/storage-reference/entities-api-query/)) will return entities ordered by their key. This property can be used to group related entities or build ad-hoc indexes.
...
The `entity().query` [method](/platform/forge/storage-reference/kvs-api-query/) used by the Custom Entity Store is eventually consistent. This means that the method returns data that may be slightly out of date.

The `entity().get` [method](/platform/forge/storage-reference/entities-api/#entity---get), on the other hand, is strictly consistent. It will always return current data.
