...
The `entity().query` [method](/platform/forge/runtime-reference/storage-api-query/) used by the Custom Entity Store is eventually consistent. This means that the method returns data that may be slightly out of date.

The `entity().get` [method](/platform/forge/runtime-reference/storage-api-custom-entities/#entity---get), on the other hand, is strictly consistent. It will always return current data.
