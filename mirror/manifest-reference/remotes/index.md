...
| `baseUrl` | `string`  or `object` (if using region specific URLs) | Yes, unless `configurable` has been specified | Base URL of the remote resource when not using region-specific URLs. |
...
| `auth` | `object` | No | An object that defines the authentication tokens to include when invoking this remote backend.  This property applies when the remote is called from a **Forge function** using [invokeRemote](/platform/forge/runtime-reference/invoke-remote-api/) from `@forge/api`.  It does **not** apply when calling the remote from a frontend module (via [invokeRemote](/platform/forge/apis-reference/ui-api-bridge/invokeRemote) from `@forge/bridge`) or from a trigger. For frontend and trigger invocations, configure `auth` on the [endpoint](/platform/forge/manifest-reference/endpoint) entry instead. |
...
| `configurable` | `object` | No | If enabled, will allow a remote to be [customer-managed](/platform/forge/customer-managed-egress-and-remotes/). |
| `configurable.name` | `string` | Yes, if using `configurable` | The name of the remote which is displayed to an admin when configuring it, as well as in Connected Apps. |
| `configurable.description` | `string` | Yes, if using `configurable` | The description of the remote which is displayed to an admin when configuring it, as well as in Connected Apps. |
| `configurable.supportedPatterns` | `array of strings` | No | An array of patterns that are acceptable for this remote. Must match the convention of `*.atlassian.com`. This is useful if only a specific domain is applicable for this remote. |
...
![App pinned to same location as Atlassian app](https://dac-static.atlassian.com/platform/forge/images/dare-pinned-app.png?_v=1.5800.2186)
...
| --- | --- |
| Adding a new `remotes` section | The app uses a new remote back end, and admins need to be explicitly notified whenever this happens, unless this is a customer-managed remote without a `baseUrl` |
...
| Changing a remote to be customer-managed by adding the `configurable` field | The app can now be configured to egress data to custom locations. |
...
| Removing a customer-managed remote's `baseUrl` | The remote back end can now effectively be switched off, which is a decrease in privilege. |
| A customer-managed remote is added without a `baseUrl` | The remote back end can't egress data until explicitly opted in to do so by an administrator. |
