...
| `licensing` | No | The app's licensing state. To enable licensing for your app, add the `enabled` field attribute and set its value to `true`.  See [Licensing overview](/platform/forge/licensing-overview/) for details on billing models, including [user-based billing](/platform/forge/adopt-user-based-billing/). See [Listing Forge apps](/platform/marketplace/listing-forge-apps/#enabling-licensing-for-your-app) for setup instructions. |
| `access` | No | Settings for user-based billing. To enable user-based billing, set `userAccess` to `true`. You must also set `licensing.enabled` to `true`.  See [Adopt user-based billing](/platform/forge/adopt-user-based-billing/) for implementation details and the [License API](/platform/forge/apis-reference/license-api/) for retrieving license information programmatically. |
