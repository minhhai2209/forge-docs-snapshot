...
![Example of a macro](https://dac-static.atlassian.com/platform/forge/snippets/images/macro-example.png?_v=1.5800.2124)
...
| `config.viewportSize` | `'small'`, `'medium'`, `'large'`, `'xlarge'`, `'max'` or `'fullscreen'` |  | The [display size](/platform/forge/manifest-reference/resources) of `resource`. Can only be set if the module is using the `resource` property. Remove this property to enable automatic resizing of the module. For `fullscreen` viewports, the `config.title` and `config.icon` will be displayed in the header. Supported in both UI Kit and Custom UI. |
