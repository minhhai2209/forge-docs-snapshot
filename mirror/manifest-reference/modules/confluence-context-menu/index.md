...
[entry in the context menu](https://developer.atlassian.com/platform/forge/ui-kit-components/confluence/context-menu/)
...
is retrieved using the [useProductContext](/platform/forge/ui-kit-hooks-reference/#useproductcontext) hook.
...
![Example of a Context menu](https://dac-static.atlassian.com/platform/forge/images/context-menu-location.png?_v=1.5800.2259)
...
| `title` | `string` or `i18n object` | Yes | The title of the context menu module, displayed as a menu item.  The `i18n object` allows for translation. See [i18n object](#i18n-object). |
...
When you register a dynamic `confluence:contextMenu` module, the `data` object uses the same properties as a static `confluence:contextMenu` module in the manifest.
...
const payload = {
  ...
  "data": {
    ...
    "description": "Runs against selected text"
  }
}
...
```
import { asApp } from "@forge/api";
const key = "context-menu-dynamic";
const payload = {
  "type": "confluence:contextMenu",
  "data": {
    "resolver": {
    ...
    "resource": "main",
    "render": "native",
    "title": "Updated dynamic context menu",
    "description": "Runs against selected text"
  }
}
