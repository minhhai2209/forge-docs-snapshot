...
**Scenario 2a: No payment method (pre-dunning)**
If there's **no valid payment method** in the billing account:

1. **Invoice generation can fail.**
   The system attempts to generate an invoice, but if a valid payment method is missing, invoice creation or payment can fail.
2. **Pre-dunning is triggered.**
   When invoice generation or payment fails, the billing system moves the apps into a **pre-dunning state**:

* During pre-dunning, we notify you that there's a problem with your billing details.
* The pre-dunning period lasts for **14 days**.
* During this time, you'll have **multiple opportunities (up to 5 retries)** to **add a payment method** and resolve the issue.
...
* Prompts billing admins to add a payment method.

**After the pre-dunning period**
If the issue is not resolved after the pre-dunning period:

* **App enforcement actions** will apply to all apps in the Developer Space, irrespective of how many apps generated the charges. This is because all apps in a Developer Space share the same invoice (a practice called co-invoicing).

  Enforcement means customers can't access your apps and see an error when they try.

To avoid pre-dunning and enforcement:

* Add a **valid payment method** before your usage exceeds the free threshold.

**Scenario 2b: Payment failure or errors (dunning)**
If you have a payment method on file, but the payment fails (for example, due to insufficient funds or a card error):

1. **Dunning is triggered.**
   When payment fails, the billing system moves the apps into a **dunning state**:

* During dunning, we repeatedly try to collect payment and notify you about the billing issue.
* The dunning period lasts for **21 days**.
* During this time, you'll have **multiple opportunities (up to 5 retries)** to **fix or update your payment method** and resolve the issue.

2. **Notifications and retries.**
   Billing admins are notified about the failed payment or missing details. The system:
...
* Keep your payment method up to date so automatic payments can succeed on the first of each month.
...
* Learn [how to monitor usage](/platform/forge/monitor-usage-metrics/), and utilize the **View usage charts** in your billing account to track how close you are to thresholds.
