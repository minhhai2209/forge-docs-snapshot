...
[persistent Forge hosted storage](/platform/forge/storage-reference/#persistent)
, as well as allowing realm pinning for apps that use [Forge Remote](/platform/forge/remote/).
...
This means that the data from the Atlassian app and all Forge apps using persistent Forge hosted storage will be hosted in the admin's chosen location. As a result:

* If an admin installs a Forge app using persistent Forge hosted storage on an Atlassian app that's pinned to a location, the app will automatically be located there too.
* If an admin migrates the data of a pinned Atlassian app to a different location, then all installed Forge apps using persistent Forge hosted storage will also be migrated there.
...
3. **The app uses a remote backend but does not store in-scope End-User Data there:** If your app interacts with remote backends solely for operations that do not involve storing in-scope End-User Data, it can be eligible for `PINNED` status. To ensure compliance, you must configure your manifest file appropriately. For information on how to do this, see [​Remotes](/platform/forge/manifest-reference/remotes/#data-residency).

By default, Forge assumes an app stores in-scope End-User Data remotely if its manifest file includes:

In addition, when an admin initiates moving their Jira or Confluence instance to a location, they'll see which apps can move at the same time. These include any eligible Forge apps that will move at the same time as the Atlassian app.

![Eligible apps moving with the Atlassian app](https://dac-static.atlassian.com/platform/forge/images/dare-apps-list.png?_v=1.5800.2211)

## In-scope End-User Data

In-scope End-User Data (EUD) refers to data that must comply with data residency requirements.
For a detailed explanation of the `inScopeEUD` property and how it affects data residency,
versioning, and Runs on Atlassian eligibility, see
...
![Atlassian app pinned to Europe](https://dac-static.atlassian.com/platform/forge/images/dare-pinned-product.png?_v=1.5800.2211)
