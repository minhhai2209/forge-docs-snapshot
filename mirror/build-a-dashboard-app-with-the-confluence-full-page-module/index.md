...
* An existing Forge app. If you don't have one, create a new app using `forge create` and select the *confluence-full-page* template.

## Step 1: Configure the manifest

Update your `manifest.yml` to configure the `confluence:fullPage` module for the dashboard app. The key properties are:
...
* Add `read:confluence-user` scope under `permissions` to fetch user information
