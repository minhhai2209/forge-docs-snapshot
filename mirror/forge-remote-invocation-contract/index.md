...
| `x-forge-oauth-system` | No | The app system token. This is used to [call Atlassian app REST APIs](/platform/forge/apis-reference/product-rest-api-reference) from your remote backend. This header is included only if `endpoint.auth.appSystemToken` or `remotes.auth.appSystemToken` is `true` in the app's manifest. Which property applies depends on how the remote is invoked: `endpoint.auth` is used for [frontend](/platform/forge/remote/calling-from-frontend) and trigger invocations, while `remotes.auth` is used for [function-to-remote](/platform/forge/remote/calling-from-function) invocations. This token has a max [TTL of 4 hours](https://developer.atlassian.com/changelog/#CHANGE-2160). |
| `x-forge-oauth-user` | No | The app's user token. This is used to [call Atlassian app REST APIs](/platform/forge/apis-reference/product-rest-api-reference) from your remote backend on behalf of a user. This header is included only if your app manifest has the following configured:  * `endpoint.auth.appUserToken` or `remotes.auth.appUserToken` is `true`. Which property applies depends on how the remote is invoked: `endpoint.auth` is used for [frontend](/platform/forge/remote/calling-from-frontend) and trigger invocations, while `remotes.auth` is used for [function-to-remote](/platform/forge/remote/calling-from-function) invocations. * At least one user scope required by your app is defined (for example, `read:confluence-content.summary` for Confluence apps).  This token has a max [TTL of 4 hours](https://developer.atlassian.com/changelog/#CHANGE-2160). It is only sent for invocations that have a user in session, so it is not delivered to [scheduled trigger](/platform/forge/remote/scheduled-triggers) invocations. |
...
| `app.license.active` | `boolean` | No | Specifies if the license is active. **Note:** `app.license.isActive` is still available but deprecated; use `app.license.active` instead. |
