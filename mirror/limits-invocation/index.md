...
| Offline user impersonation tokens | 1,000 requests per minute, per app | The maximum number of requests per minute that an app can make to request offline user impersonation tokens using the [offlineUserAuthToken](/platform/forge/remote/calling-product-apis/#offline-user-impersonation) mutation. |
