...
[Get help](https://community.developer.atlassian.com/session/sso?return_path=%2Fnew-topic%3Fcategory_id%3D45%26tags%3Dforge-build-a-custom-ui-app-in-jira)
...
At the end of this tutorial, you’ll have created a Forge app in Jira that uses Custom UI to display
...
The `jira-issue-panel-custom-ui` template has React JS for the static frontend and Node JS
...
* `manifest.yml`: Describes your app. It contains the name and ID of your app, along with the modules
  it uses. This app displays an issue panel on all Jira issues and has a resource that provides
  the content of your Custom UI for the panel.
...
This app displays content in a Jira issue panel using the `jira:issuePanel` module. Jira shows the
title of the `jira:issuePanel` as the panel's heading. Let's change the title to include your name.

1. In the app’s top-level directory, open the `manifest.yml` file.
2. Find the `title` entry under the `jira:issuePanel` module.
3. Change the value of `title` from `hello-world-custom-ui` to `Hello World from <your name>`.
...
message in a Jira issue panel by serving a single-page React app.
...
![A Jira issue displaying a Custom UI forge app](https://dac-static.atlassian.com/platform/forge/images/forge-getting-started-jira-custom-ui.png?_v=1.5800.1877)
...
By now, you can now see your app displaying the message `Hello World from <your name>` in the Jira issue panel.
This is the result of setting up your static assets & using a resolver function to dynamically
...
5. Navigate to the app's top-level directory and start a tunnel for your app by running:
   ...
   You have to create a resources definition for this to work. This is described under
   *Connecting the tunnel to your own dev server* on the [Tunneling](/platform/forge/tunneling) page.
...
4. Navigate to the app's top-level directory and start a tunnel for your app by running:
   ...
   You have to create a resource definition for this work. This is described under
   *Connecting the tunnel to your own dev server* on the [Tunneling](/platform/forge/tunneling) page.
